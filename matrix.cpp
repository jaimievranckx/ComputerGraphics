...
Matrix::Matrix(Affine type, Vector3 v, Vector3 extra) {
	mat.assign(4,vector<double>(4,0));
	for (int i=0;i<4;i++) mat[i][i] = 1;
	switch(type) {
		case SHEAR:
...
Matrix::Matrix(Affine type, Vector3 v) {
	mat.assign(4,vector<double>(4,0));
	for (int i=0;i<4;i++) mat[i][i] = 1;
	switch(type) {
		case SCALE:
...
Vector3 Matrix::transform(Vector3 point, int w) const {
    Vector3 temp;
	temp.x = point.dot(Vector3(mat[0][0],mat[0][1],mat[0][2])) + mat[0][3]*w;
	temp.y = point.dot(Vector3(mat[1][0],mat[1][1],mat[1][2])) + mat[1][3]*w;
	temp.z = point.dot(Vector3(mat[2][0],mat[2][1],mat[2][2])) + mat[2][3]*w;
	return temp;
}
